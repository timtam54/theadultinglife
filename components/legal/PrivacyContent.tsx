/*
 * Privacy Policy body copy — the single source of truth.
 *
 * Rendered by both the standalone /privacy page and the in-app consent
 * dialog so the two never drift out of sync.
 */

export const PRIVACY_LAST_UPDATED = "30 August 2026";
...
<h2>Where your data lives &amp; how it&apos;s protected</h2>
<p>
  <strong>Your information stays in Australia.</strong>{" "}
  Everything you type or upload — records, form answers, uploaded
  files, Planner entries, receipts — is stored on Australian
  servers in Sydney. It&apos;s encrypted both while it&apos;s
  travelling to us (HTTPS) and while it&apos;s sitting on the
  server (at rest). Only you, and anyone you&apos;ve explicitly
  shared an item with, can read it.
</p>
<details className="not-prose rounded-xl border border-tal-line bg-tal-cream-soft/60 p-4 my-4">
  <summary className="cursor-pointer text-sm font-medium text-tal-plum">
    Technical detail — the specifics
  </summary>
  <div className="mt-3 space-y-3 text-sm text-tal-plum leading-relaxed">
    <p>
      <strong>Database &amp; files.</strong>{" "}
      We use Supabase for the database (Postgres) and file storage,
      both hosted in the <em>ap-southeast-2 (Sydney)</em> region.
      Data rows are encrypted at rest using AES-256. Files
      (photos, scans, PDFs) are stored in a separate encrypted
      object store and are accessed via short-lived signed URLs —
      the file path alone isn&apos;t enough to read them.
    </p>
    <p>
      <strong>App servers.</strong>{" "}
      The Adulting Life web app runs on Vercel, configured to the
      Sydney (<em>syd1</em>) region so your requests don&apos;t
      leave Australia in normal use.
    </p>
    <p>
      <strong>Access controls.</strong>{" "}
      Every row in the database is protected by Postgres
      row-level-security policies — the server code physically
      can&apos;t return another user&apos;s data even by accident.
      When you share an item, we record the grant as a separate
      row and re-check it on every read.
    </p>
    <p>
      <strong>In transit.</strong>{" "}
      All traffic between your browser (or phone) and our servers
      uses TLS 1.2 or higher. The service worker (when the app is
      installed as a PWA) caches only pages you&apos;ve already
      viewed, on your own device.
    </p>
    <p>
      <strong>Backups.</strong>{" "}
      Supabase runs daily encrypted backups of the database,
      retained inside the same Sydney region. If we ever needed
      to restore, no data would leave Australia.
    </p>
    <p>
      <strong>Third parties.</strong>{" "}
      A few features route small amounts of data to specialist
      providers (see &ldquo;Third-party services we use&rdquo;
      below). Where that happens, we&apos;ve chosen providers
      with strong privacy commitments and, wherever possible,
      Australian or Australia-friendly data-handling terms.
    </p>
  </div>
</details>
