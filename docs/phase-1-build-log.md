Quote
Official quote as sent to Donna — $15,000 + GST at $80/hr, billed as four fortnightly invoices of $3,750 + GST ($4,125) each, paid in arrears (you deliver two weeks first, then invoice).
The four two-week blocks, in the revised order you settled on (plumbing front-loaded into Fortnight 1):
Fortnight 1 (Weeks 1–2) — Foundation + all platform plumbing. Login (Google/MS/Apple), Azure SQL, deploy pipeline, custom Adulting Life styling and links to the site, plus a click-through mockup of all three sections on day one. Then Square billing (subscriptions, trial, grace/lockout, plan gating), auditing/usage analytics dashboard, and the AI layer (feedback + speech-to-text). Invoice: $3,750 + GST.
Fortnight 2 (Weeks 3–4) — Module 1: Life Administration Organisation. The five categories, structured record forms, search/filter, expiry reminders — built end-to-end. Invoice: $3,750 + GST.
Fortnight 3 (Weeks 5–6) — Module 2: Secure Document & Information Storage. Upload, attach-to-record, viewer/download/replace/delete, storage view. Invoice: $3,750 + GST.
Fortnight 4 (Weeks 7–8) — Module 3: Educational Tools + finalise, QA, launch. Content/quizzes/progress, billing tightening, final QA and go-live. Invoice: $3,750 + GST.
The six line items behind the $15,000: Foundation $3,520, three core sections, billing $1,920, auditing $1,280, AI assistance $1,600. The DV section is explicitly excluded from this phase and needs its own discovery conversation before quoting.
Note one drift point to keep straight: the final architecture you landed on moved off Azure Blob to Supabase (Postgres) + Vercel, with Supabase Storage replacing Azure Blob — so if you're re-sending or referencing the quote, the client-facing wording stays plain (no stack jargon) but the doc itself predates that switch.
Want me to pull up the exact schedule doc, or regenerate a clean quote PDF with your AussieSoft/ABN header?
